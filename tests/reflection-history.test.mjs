...
test("keeps legacy insights while removing the external observer label", () => {
  assert.equal(
    ...
    "Ты возвращаешься к теме запуска и проверяешь сроки.",
  );
  assert.equal(
    neutralizeExternalObserverVoice(
...
test("turns legacy observer copy into grammatical direct language", () => {
  assert.equal(
    neutralizeExternalObserverVoice(
      "Продуктивность автора напрямую зависит от наличия заранее определенных задач.",
    ),
    "Твоя продуктивность напрямую зависит от наличия заранее определенных задач.",
  );
  assert.equal(
    neutralizeExternalObserverVoice(
      "Автор видит причину трудностей в отсутствии глобальной цели.",
    ),
    "Ты видишь причину трудностей в отсутствии глобальной цели.",
  );
  assert.equal(
    neutralizeExternalObserverVoice(
      "Сомнения мешают автору подготовиться к встрече, хотя он признает ценность своих наработок.",
    ),
    "Сомнения мешают тебе подготовиться к встрече, хотя ты признаёшь ценность своих наработок.",
  );
});

test("uses the highest-ranked insight in the capture preview", () => {
  assert.equal(
    buildPrimaryInsightPreview({
      summary: "Автор анализирует причины недельного перерыва в записях.",
      insights: [
        "Продуктивность автора напрямую зависит от наличия заранее определенных задач; отсутствие четкого плана ведет к фрустрации и поиску отвлечений.",
        "Отказ от использования телефона сразу после пробуждения ускоряет процесс перехода в бодрое состояние.",
        "Автор видит причину своих профессиональных и личных трудностей в отсутствии глобальной цели и дисциплины, а не только во внешних обстоятельствах.",
        "Сомнения в ценности собственных наработок мешают автору подготовиться к определяющей встрече, хотя он признает значимость полученных навыков.",
      ],
      themes: ["Самодисциплина"],
    }),
    "Твоя продуктивность напрямую зависит от наличия заранее определенных задач; отсутствие четкого плана ведет к фрустрации и поиску отвлечений.",
  );
});
